...
describe('DemographicsFormPage', () => {
  ...
  it('should display error message when template fetch fails', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/failed to load demographics form/i)).toBeInTheDocument();
    });
  });
  ...
  it('should display warning when template is not available', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/demographics form template not available/i)).toBeInTheDocument();
    });
  });
