...
describe('ProfilePage', () => {
  ...
  it('displays event code input form', async () => {
    vi.mocked(global.fetch)
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ success: true, data: { id: 'user-1', email: 'test@example.com' } }),
      } as Response)
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ success: true, events: [] }),
  ...
  it('submits event code successfully', async () => {
    ...
    vi.mocked(global.fetch)
      ...
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ success: true, message: 'Successfully registered for event' }),
      } as Response);
    ...
    await waitFor(() => {
      expect(screen.getByText('Successfully registered for event')).toBeInTheDocument();
    });
  });
  ...
  it('displays empty state when no events registered', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/No events registered yet. Join an event using a code above./)
      ).toBeInTheDocument();
    });
  });
