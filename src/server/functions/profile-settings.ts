...
import {adminDb, serverTimestamp} from '@/lib/firebase/firebase.admin'
 ...
 */
export const updateProfileWithHistoryFn = createServerFn({method: 'POST'})
	.inputValidator((d: unknown) => {
		const input = d as {data: unknown; expectedVersion?: number}
		const result = updateProfileSchema.safeParse(input.data)
		if (!result.success) {
	...
	.handler(
		async ({data}): Promise<{success: boolean; updatedFields: string[]; newVersion: number}> => {
			const user = await requireAuth()
 ...
 * @throws ConflictError if version mismatch detected
 */
export const updateDemographicsWithHistoryFn = createServerFn({method: 'POST'})
	.inputValidator((d: unknown) => {
		const input = d as {data: unknown; expectedVersion?: number}
		const result = demographicsSchema.safeParse(input.data)
		if (!result.success) {
			throw new ValidationError(result.error.issues[0]?.message ?? 'Invalid input')
		}
		return {
